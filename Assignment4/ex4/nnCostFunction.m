...
a = [ones(t, 1) a];
h = (sigmoid((Theta2 *a')));
JJ = zeros(num_labels,1);

z2 = X * Theta1';
for i = 1:num_labels
    JJ(i) = (1/m)*((-y_out(i,:)*(log(h(i,:)))') - ((1-y_out(i,:))*(log(1- h(i,:)))'));
   end
...
Theta1sq = Theta1.^2;
Theta2sq = Theta2.^2;
Thetasum = (sum(sum(Theta1sq))) + (sum(sum(Theta2sq))) - sum(Theta1sq(:,1)) - sum (Theta2sq(:,1));
...
Theta1_grad(:,i) = Theta1_grad(:,i) + (lambda*Theta1(:,i))./m;
end

for i = 2:(size(Theta2,2))
Theta2_grad(:,i) = Theta2_grad(:,i) + (lambda*Theta2(:,i))./m;
end 
